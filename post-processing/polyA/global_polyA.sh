#!/usr/bin/env bash
#the purpose of this file is to extract the polyA tags from the aligned bam files 
#and to store them in a tab separated table, where column1=unique_read_id and column2=polyAlength (as determined by dorado during the basecalling step)
ml samtools

function extract_all {
	small=$( basename $1 | sed 's/GRCh38//;s/basecall//;s/.bam//' | tr -s "." | sed -e 's/0.7.2./0.7.2/' )
	echo $small
 	#the format between RNA002 and RNA004 is different, thus we allocate chemistry via the filename and process accordingly
	case "$1" in
		*"002"*)
